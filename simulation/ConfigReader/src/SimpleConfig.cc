...
namespace crd {
  ...
  void SimpleConfig::ReadFile( const string& filename,
                    const string basefold,
                    bool allowReplacement,
                    bool messageOnReplacement,
                    bool messageOnDefault
  ...
  void SimpleConfig::processInclude( const std::string& line){
    ...
    std::string fname = line.substr(j0+1,j1-j0-1);

    // Read the included file; copy verbosity control from this object.
    SimpleConfig nestedFile(fname,_basefold,_allowReplacement,_messageOnReplacement,_messageOnDefault);
